...
class ConfigurationMemoryIO[T <: Data](params: ConfigurationMemoryParams[T]) extends Bundle {
  ...
  // SVM Intercept "valid" is actually not working.
...
class ConfigurationMemory[T <: chisel3.Data : Real : Order : BinaryRepresentation](val params: ConfigurationMemoryParams[T]) extends Module {
  ...
  val addr = io.in.bits.asUInt()(io.in.bits.asUInt().getWidth-1,io.in.bits.asUInt().getWidth-2)
  ...
  val pcaVectorMemoryParams = new MemoryBufferParams[T] {
    override val protoData: T = params.protoData
  ...
  pcaVectorMemory.io.in.bits := io.in.bits
  ...
  svmSupportVectorMemory.io.in.bits := io.in.bits
  ...
  svmAlphaVectorMemory.io.in.bits := io.in.bits
  ...
  svmInterceptMemory.io.in.bits := io.in.bits
