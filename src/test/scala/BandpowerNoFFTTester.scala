...
/**
  ...
  */
class GoldenDoubleBandpowerNoFFT(nBins: Int, idxStartBin: Int, idxEndBin: Int, dataType: String) {
...
/**
  ...
  * @tparam T
  */
class BandpowerNoFFTTester[T <: Data](c: BandpowerNoFFT[T], params: BandpowerNoFFTParams[T], dataBP: Int) extends DspTester(c) {
  ...
  val bandpower = new GoldenDoubleBandpowerNoFFT(nBins, idxStartBin, idxEndBin, dataType)
...
object UIntBandpowerNoFFTTester {
  def apply(params: BandpowerNoFFTParams[UInt], debug: Int): Boolean = {
    if (debug == 1) {
      chisel3.iotesters.Driver.execute(Array("-tbn", "firrtl", "-fiwv"), () => new BandpowerNoFFT(params)){
        c => new BandpowerNoFFTTester(c, params, 0)
      }
    } else {
      dsptools.Driver.execute(() => new BandpowerNoFFT(params), TestSetup.dspTesterOptions) {
        c => new BandpowerNoFFTTester(c, params, 0)
      }
    }
  }
}
object FixedPointBandpowerNoFFTTester {
  def apply(params: BandpowerNoFFTParams[FixedPoint], dataBP: Int, debug: Int): Boolean = {
    if (debug == 1) {
      chisel3.iotesters.Driver.execute(Array("-tbn", "firrtl", "-fiwv"), () => new BandpowerNoFFT(params)){
        c => new BandpowerNoFFTTester(c, params, dataBP)
      }
    } else {
      dsptools.Driver.execute(() => new BandpowerNoFFT(params), TestSetup.dspTesterOptions) {
        c => new BandpowerNoFFTTester(c, params, dataBP)
