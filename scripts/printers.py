...
def print_array_to_file(f,array_name,part):
...
def print_matrices (f, name, matrix):
    for i in range(matrix.shape[1]):
        f.write("double %s%d[] = {" % (name,i))
        if len(matrix[:,i]) >= 1000:
            for j in range(math.floor(len(matrix[:,i]) / 1000)):
                print_array_to_file(f,matrix[j*1000:((j+1)*1000)-1,i],'mid')
        else: j = -1
        print_array_to_file(f,matrix[(j+1)*1000:,i],'end')
...
def generate_files(X_test_raw, y_test_raw, fs, 
    ...
    if normalize == 1:
        ...
        f.write("mean = ")
        print_array_to_file(f,X_train_mean,'all')
        
        f.write("recip_var = ")
        print_array_to_file(f,1/X_train_var,'all')
        f.close()
    else:
    ...
    f.write("taps = ")
    print_array_to_file(f,lpf,'scala')
    f.close()
    ...
    if (class_type == "ecoc"):
        ...
        print_array_to_file(f,clf.code_book_,'scala')
        f.close()
    else:
